...
inline void KeyFromUint128(uint128 fp, string* key) {
  uint64_t norder[] = { htonll(Uint128High64(fp)),
...
inline uint32_t KeyToUint32(StringPiece key) {
  uint32_t value;
...
inline uint64_t KeyToUint64(StringPiece key) {
  uint64_t value;
  DCHECK_EQ(key.size(), sizeof(value));
  memcpy(&value, key.data(), sizeof(value));
  return ntohll(value);
...
inline string Int32ToKey(int32_t i32) {
  string s;
  KeyFromInt32(i32, &s);
  return s;
}
...
inline string EncodeUint32(uint32_t i) {
  return EncodePOD(i);
}

inline string EncodeUint64(uint64_t i) {
  return EncodePOD(i);
}
...
inline bool DecodeUint32(StringPiece s, uint32_t* i) {
  return DecodePOD(s, i);
}

inline bool DecodeUint64(StringPiece s, uint64_t* i) {
  return DecodePOD(s, i);
}
