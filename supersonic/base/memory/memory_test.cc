...
TEST(BufferAllocatorTest, TrivialAllocatorAllocates) {
  unique_ptr<Buffer> buffer(HeapBufferAllocator::Get()->Allocate(1000));
...
TEST(BufferAllocatorTest, ClearingAllocatorClears) {
  ...
  unique_ptr<Buffer> buffer(allocator.Allocate(10));
  for (int i = 0; i < 10; ++i) {
...
TEST(BufferAllocatorTest, SharedQuotaAllocatorShouldAllocate) {
  ...
  EXPECT_EQ(0, quota.GetUsage());
  EXPECT_EQ(1000, allocator->Available());
  unique_ptr<Buffer> buffer5(allocator->BestEffortAllocate(1200, 800));
  ...
  quota.SetQuota(1200);
  EXPECT_EQ(200, allocator->Available());
  unique_ptr<Buffer> buffer6(allocator->BestEffortAllocate(1200, 100));
...
TEST(BufferAllocatorTest, QuotaAllocatorWorksWithMemoryLimit) {
  ...
  unique_ptr<BufferAllocator> allocator(new MemoryLimit(50));
  unique_ptr<BufferAllocator> outer_allocator(new MediatingBufferAllocator(
      allocator.get(), &quota));

  EXPECT_EQ(0, quota.GetUsage());
  unique_ptr<Buffer> buffer(outer_allocator->BestEffortAllocate(80, 20));
...
TEST(BufferAllocatorTest, MaxSizeTAllocateRequestFailsGracefully) {
  ...
  unique_ptr<BufferAllocator> allocator(new MediatingBufferAllocator(
      HeapBufferAllocator::Get(), &quota));
  EXPECT_EQ(1000, allocator->Available());
  unique_ptr<Buffer> buffer1(allocator->BestEffortAllocate(400, 100));
  EXPECT_EQ(400, buffer1->size());
  EXPECT_EQ(600, allocator->Available());
  const size_t max_size_t = numeric_limits<size_t>::max();
  unique_ptr<Buffer> buffer2(allocator->Allocate(max_size_t));
...
TEST(BufferAllocatorTest, SoftQuotaAllocateSucceedsEvenWhenAvailableIs0) {
  ...
  EXPECT_EQ(400, buffer1->size());
  EXPECT_EQ(600, allocator->Available());
  unique_ptr<Buffer> buffer2(allocator->BestEffortAllocate(400, 100));
  EXPECT_EQ(400, buffer2->size());
  EXPECT_EQ(200, allocator->Available());
  EXPECT_EQ(800, quota.GetUsage());
  {
    unique_ptr<Buffer> buffer3(allocator->BestEffortAllocate(400, 100));
    EXPECT_EQ(200, buffer3->size());
    EXPECT_EQ(0, allocator->Available());
    EXPECT_EQ(1000, quota.GetUsage());
    // Available() is 0, but this is a soft/unenforced quota, so we should be
    // able to allocate anyway.
    unique_ptr<Buffer> buffer4(allocator->BestEffortAllocate(400, 100));
...
TEST(BufferAllocatorTest, CompoundSoftHardQuota) {
  ...
  unique_ptr<BufferAllocator> hard_allocator(new MediatingBufferAllocator(
      HeapBufferAllocator::Get(), &hard_quota));
  unique_ptr<BufferAllocator> compound_allocator(new MediatingBufferAllocator(
      hard_allocator.get(), &soft_quota));
  EXPECT_EQ(500, compound_allocator->Available());
  unique_ptr<Buffer> buffer1(compound_allocator->BestEffortAllocate(800, 400));
  EXPECT_EQ(500, buffer1->size());
  EXPECT_EQ(0, compound_allocator->Available());
  unique_ptr<Buffer> buffer2(compound_allocator->BestEffortAllocate(800, 400));
  EXPECT_EQ(400, buffer2->size());
  EXPECT_EQ(0, compound_allocator->Available());
  unique_ptr<Buffer> buffer3(compound_allocator->BestEffortAllocate(800, 400));
...
TEST(BufferAllocatorTest, AvailableInMemoryLimit) {
  ...
  unique_ptr<Buffer> buffer(limit.Allocate(300));
...
TEST(BufferAllocatorTest, ReallocShouldAdjustQuota) {
  ...
  unique_ptr<Buffer> buffer(limit2.Allocate(300));
...
TEST(BufferAllocatorTest, AllocatingEmptyAlwaysSucceeds) {
  MemoryLimit limit(0);
  unique_ptr<Buffer> buffer1(limit.BestEffortAllocate(300, 10));
  EXPECT_TRUE(buffer1.get() == NULL);
  unique_ptr<Buffer> buffer2(limit.BestEffortAllocate(300, 0));
  ASSERT_TRUE(buffer2.get() != NULL);
  EXPECT_EQ(0, buffer2->size());
  unique_ptr<Buffer> buffer3(limit.Allocate(0));
...
TEST(BufferAllocatorTest, ReallocatingEmptyAlwaysSucceeds) {
  ...
  unique_ptr<Buffer> buffer1(limit.BestEffortAllocate(400, 300));
...
TEST(BufferAllocatorTest, SoftQuotaBypassing) {
  ...
  unique_ptr<Buffer> buffer1(lifted_quota.BestEffortAllocate(50, 20));
  ASSERT_TRUE(buffer1.get() != NULL);
  EXPECT_EQ(50, buffer1->size());
  unique_ptr<Buffer> buffer2(lifted_quota.BestEffortAllocate(100, 20));
  ASSERT_TRUE(buffer2.get() != NULL);
  EXPECT_EQ(100, buffer2->size());
  unique_ptr<Buffer> buffer3(lifted_quota.BestEffortAllocate(100, 20));
  ASSERT_TRUE(buffer3.get() != NULL);
...
TEST(BufferAllocatorTest, GuaranteeMemoryTest) {
  ...
  {
    unique_ptr<Buffer> buffer1(less_memory.Allocate(30));
    ASSERT_TRUE(buffer1.get() != NULL);
    ...
    unique_ptr<Buffer> buffer2(less_memory.Allocate(20));
  ...
  {
    unique_ptr<Buffer> buffer1(more_memory.Allocate(70));
    ASSERT_TRUE(buffer1.get() != NULL);
    ...
    EXPECT_EQ(80, more_memory.Available());
    unique_ptr<Buffer> buffer2(more_memory.Allocate(30));
    ASSERT_TRUE(buffer2.get() != NULL);
...
TEST(BufferAllocatorTest,
  ...
  unique_ptr<Buffer> buffer1(stats_allocator.BestEffortAllocate(50, 50));
  ASSERT_TRUE(buffer1.get() != NULL);
  unique_ptr<Buffer> buffer2(stats_allocator.BestEffortAllocate(150, 100));
  ASSERT_TRUE(buffer2.get() != NULL);
